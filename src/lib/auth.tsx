...
const persistAuthToken = async (firebaseUser: FirebaseUser | null) => {
  if (firebaseUser) {
    try {
      const token = await getIdToken(firebaseUser, true);
      // Set the token as a cookie that expires in 14 days
      Cookies.set('firebase-auth-token', token, { expires: 14, secure: process.env.NODE_ENV === 'production' });
      console.log('Auth token persisted in cookies');
    } catch (error) {
      console.error('Error persisting auth token:', error);
    }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const register = async (userData: RegisterUserData) => {
    try {
      ...
      if (userData.referredBy) {
        try {
          console.log('Processing referral bonus for:', userData.referredBy);
          ...
          if (!referrerQuery.empty) {
            ...
            // Add referral bonus (10 credits)
            const referralBonus = 10;
            await updateDoc(doc(db, 'users', referrerData.id), {
              credits: (referrerData.credits || 0) + referralBonus,
              updatedAt: Timestamp.now()
            });
            ...
            console.log(`Referral bonus of ${referralBonus} credits added to ${referrerData.id}`);
          } else {
            console.log('No user found with referral code:', userData.referredBy);
          }
        } catch (referralError) {
  ...
  return (
    <AuthContext.Provider value={{ user, isLoading, login, logout, register, updateUserProfile, updateProfileImage, changePassword }}>
      {authInitialized ? children : (
        <div className="min-h-screen flex items-center justify-center">
          <div className="w-12 h-12 border-4 border-[#D91CD2] border-t-transparent rounded-full animate-spin"></div>
          <p className="ml-4 text-lg">Initializing authentication...</p>
