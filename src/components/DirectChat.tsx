...
export default function DirectChat({ className = '' }: DirectChatProps) {
  ...
  // Real-time message listening for active conversation
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      loadMessages(activeConversation);
      loadConversations(); // Update conversation list for unread counts
    }, 3000);

    return () => clearInterval(interval);
  }, [activeConversation, user]);

  // General polling for conversations when no active conversation
  useEffect(() => {
    if (!user) return;
                    ...
                    <p className="text-xs text-gray-400 truncate">
                      {conversation.lastMessage 
                        ? (typeof conversation.lastMessage === 'string' 
                            ? conversation.lastMessage 
                            : conversation.lastMessage.message || t('newMessage'))
                        : t('No messages yet')}
                    </p>
                  </div>
                </motion.div>
            ...
            <div className="border-t border-gray-800 p-4 bg-gray-900/80 backdrop-blur-sm">
              <form onSubmit={sendMessage} className="flex items-end space-x-3">
                ...
                <button
                  ...
                  className="p-4 mb-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-xl hover:from-purple-600 hover:to-pink-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105 disabled:hover:scale-100"
...
import { useState, useEffect, useRef } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { FiSend, FiSearch, FiUser, FiMessageCircle, FiClock, FiImage, FiX, FiLoader } from 'react-icons/fi';
import { DirectMessage, User } from '@/types';
import { directMessageService, userService } from '@/lib/database';
import { useAuth } from '@/lib/auth';
import Image from 'next/image';
  ...
  if (!user) {
    return (
      <div className="card text-center py-12">
        ...
        <p className="text-gray-400 mb-6">Sign in to chat with other users!</p>
                ...
                <div className="w-10 h-10 rounded-full flex items-center justify-center mr-3 overflow-hidden">
                  {activeUserData?.profileImage ? (
                    <Image
                      ...
                      className="w-10 h-10 rounded-full object-cover"
                    />
                  ) : activeUserData?.role === 'admin' && activeUserData?.customEmoji ? (
                    <Image
                    ...
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${
                      activeUserData?.role === 'admin' ? 'bg-purple-600' :
                ...
                <div>
                  <h3 className="font-medium flex items-center">
                    ...
                    {activeUserData?.role === 'admin' && (
                      <span className="ml-2 text-xs bg-purple-600 px-1 rounded">Admin</span>
                    )}
                    {activeUserData?.role === 'coach' && (
                      <span className="ml-2 text-xs bg-[#D91CD2] px-1 rounded">Coach</span>
                    )}
                  </h3>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center p-8">
              ...
              <p className="text-gray-400">
                Choose an existing conversation or search for a user to start chatting
              </p>
            </div>
          </div>
        )}
