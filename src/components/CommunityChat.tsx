...
export default function CommunityChat({ courseId, courseName }: CommunityChatProps) {
  ...
  if (!user) {
    return (
      <div className="card text-center py-12">
        ...
        <p className="text-gray-400 mb-6">Sign in to chat with your classmates and coach!</p>
  ...
  return (
    <div className="card h-[600px] flex flex-col">
      ...
      <div className="border-b border-gray-800 p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-400">
            ...
            <div className="flex items-center space-x-1">
              <FiUsers size={16} />
              <span>{messages.length > 0 ? new Set(messages.map(m => m.senderId)).size : 0} members</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-900 max-h-[400px] md:max-h-[450px]">
        {messages.length === 0 ? (
          <div className="text-center py-12">
            <FiMessageCircle className="mx-auto text-4xl text-gray-500 mb-4" />
            <h4 className="text-lg font-medium mb-2">Start the conversation!</h4>
            <p className="text-gray-400">Be the first to say hello to your classmates.</p>
          </div>
        ) : (
          <AnimatePresence>
            {messages.map((message, index) => {
                      ...
                      <div className="flex items-center space-x-2 mb-2">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm font-medium text-[#D91CD2]">
                            ...
                            {message.senderRole === 'coach' && ' (Coach)'}
                          </span>
                        </div>
                        <span className="text-xs text-gray-500">{formatMessageTime(message.timestamp)}</span>
                      </div>
