...
import { FiCreditCard, FiDollarSign, FiX, FiCheck, FiCamera, FiPercent } from 'react-icons/fi';
  ...
  const loadPaymentSettings = async () => {
    try {
      ...
      const methods: PaymentMethod[] = [
        ...
        {
          id: 'gift-card',
          name: t('Scan Gift Card'),
          icon: <FiCamera size={24} />,
          isEnabled: true,
          description: t('Scan QR code or upload gift card image')
        },
        {
          id: 'discount-card',
          name: t('Scan Discount Card'),
          icon: <FiPercent size={24} />,
          isEnabled: true,
          description: t('Scan coach discount card for automatic discount')
        }
      ];

      // Add credits option if user has sufficient balance
      if (user && user.credits >= discountedAmount) {
        methods.unshift({
          ...
          isEnabled: true,
          description: t('useCreditBalanceDescription', {
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-2 sm:p-4">
          ...
          >
            ...
            <div className="flex justify-between items-center p-4 sm:p-6 border-b border-gray-700">
              <h3 className="text-lg sm:text-xl font-semibold truncate pr-2">{props.title}</h3>
              <button
