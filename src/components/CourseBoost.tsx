...
export default function CourseBoost({ course, onBoostSuccess, className = '' }: CourseBoostProps) {
  ...
  const [boostPlans, setBoostPlans] = useState({
    ...
    premium: {
      ...
      duration: '7 days',
      features: [
        'Top position in "Featured" section',
        'Badge on course card',
    ...
    <div className={className}>
      ...
      {course.boosted && remainingTime ? (
        <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-black p-3 rounded-lg flex items-center justify-between">
          ...
          <span className="text-sm">{remainingTime} left</span>
        </div>
      ) : (
        <button
          onClick={() => setShowBoostModal(true)}
        ...
        {showBoostModal && (
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <div>
                  <h3 className="text-2xl font-bold">Boost Your Course</h3>
                  <p className="text-gray-400">Increase visibility and attract more students</p>
                </div>
                <button
                  onClick={() => setShowBoostModal(false)}
                  className="text-gray-400 hover:text-white"
              ...
              <div className="bg-blue-900/30 border border-blue-700 rounded-lg p-4 mb-6">
                <div className="flex items-start space-x-3">
                  <FiInfo className="text-blue-400 mt-1" />
                  <div>
                    <h4 className="font-medium text-blue-300">How Course Boost Works</h4>
                    <p className="text-sm text-blue-200 mt-1">
                      Boosted courses appear at the top of search results and in featured sections, 
                      giving them maximum visibility to potential students.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800 rounded-lg p-4 mb-6">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span>Your Credits: ${user?.credits || 0}</span>
                  <span className={`${
                    (user?.credits || 0) >= boostPlans[selectedPlan].price 
                      ? 'text-green-400' 
                      : 'text-red-400'
                    ...
                    {(user?.credits || 0) >= boostPlans[selectedPlan].price 
                      ? 'Sufficient credits' 
                      : 'Insufficient credits - card payment required'
                    }
                  </span>
                </div>
              </div>
              ...
              <div className="flex space-x-4">
                ...
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-[#D91CD2] to-[#7B1FA2] hover:from-[#E63DE6] hover:to-[#8E24AA] text-white py-3 rounded-lg transition-all disabled:opacity-50"
                >
                  {isProcessing ? 'Processing...' : `Boost for $${boostPlans[selectedPlan].price}`}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      <PaymentHandler
        ...
        amount={boostPlans[selectedPlan].price}
        title="Course Boost Payment"
        description={`Pay for ${boostPlans[selectedPlan].name} - ${boostPlans[selectedPlan].duration}`}
        userId={user?.id || ''}
      />
    </div>
  );
