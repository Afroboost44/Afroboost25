  ...
  const submitReview = async () => {
    ...
    try {
      ...
      await notificationService.create({
        ...
        message: `${user.firstName} ${user.lastName} left a ${rating}-star review for "${course.title}".`,
        type: 'review',
        read: false
      });
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-200">Help others by leaving a review</p>
            </div>
            <button
              onClick={() => setShowReviewModal(true)}
              className="bg-white text-[#D91CD2] px-4 py-2 rounded-lg font-medium hover:bg-gray-100 transition-colors"
          ...
          <div className="flex items-center justify-between">
            ...
            {reviews.length > 0 && (
              <div className="flex items-center space-x-2">
                ...
                <span className="text-gray-400">
                  {course?.averageRating?.toFixed(1)} ({reviews.length} reviews)
                </span>
              </div>
            )}
          </div>
