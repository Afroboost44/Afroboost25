...
// Function to get PayPal access token
async function getPayPalAccessToken(clientId: string, clientSecret: string, isProduction: boolean = false) {
  const baseUrl = isProduction ? 'https://api.paypal.com' : 'https://api.sandbox.paypal.com';
...
// Function to capture PayPal order using REST API
async function capturePayPalOrder(orderId: string, clientId: string, clientSecret: string, isProduction: boolean = false) {
  try {
    // Get access token
...
export async function POST(request: Request) {
  try {
    ...
    // Capture PayPal payment using actual API
    const captureResult = await capturePayPalOrder(orderId, clientId, clientSecret, isProduction || false);
