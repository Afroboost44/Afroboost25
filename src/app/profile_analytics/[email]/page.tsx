...
export default function ProfileAnalytics({ params }: { params: Promise<{ email: string }> }) {
  ...
  useEffect(() => {
    if (!authLoading && currentUser && decodedEmail) {
      // Allow if viewing own profile or if admin
      if (currentUser.email !== decodedEmail && currentUser.role !== 'admin') {
        router.push('/dashboard');
      }
    }
  }, [currentUser, decodedEmail, router, authLoading]);
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold gradient-text mb-8">User not found</h1>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <motion.div
        ...
        transition={{ duration: 0.5 }}
      >
        <h1 className="text-3xl font-bold gradient-text mb-8">Profile Analytics</h1>
        ...
        <Card className="mb-8">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-2xl font-bold mb-2">{user.firstName} {user.lastName}</h2>
              <p className="text-gray-300 mb-2">{user.email}</p>
              <p className="text-gray-400 mb-4">Role: <span className="capitalize">{user.role}</span></p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {activeTab === 'overview' && (
            <>
              ...
              <Card className="lg:col-span-2">
                <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
                  <FiActivity className="text-[#D91CD2]" /> Booking Activity
                </h3>
                <div className="h-80">
                  {typeof window !== 'undefined' && (
                    <LineChart
              ...
              <Card>
                <h3 className="text-xl font-semibold mb-4 flex items-center gap-2">
                  <FiBarChart2 className="text-[#D91CD2]" /> Booking Status
                </h3>
                <div className="h-80">
                  {typeof window !== 'undefined' && (
                    <DonutChart
                    ...
                    .map((item, index) => {
                      ...
                      return (
                        <div key={index} className="flex items-center gap-4 p-3 rounded-lg bg-black/30 border border-[#D91CD2]/20">
                          ...
                          <div>
                            <p className="font-medium">
                              {isBooking ? `Booking ${(item as Booking).status}` : `${(item as Transaction).type} - ${(item as Transaction).status}`}
                            </p>
                            <p className="text-sm text-gray-400">
                              {formatDate(item.createdAt)}
                            </p>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[#D91CD2]/30">
                        ...
                        <th className="text-left py-3 px-4">Coach ID</th>
                    ...
                    <tbody>
                      {bookings.map((booking, index) => (
                        <tr key={index} className="border-b border-[#D91CD2]/10">
                          ...
                          <td className="py-3 px-4">{booking.courseId}</td>
                          <td className="py-3 px-4">{booking.coachId}</td>
                          <td className="py-3 px-4">${booking.paymentAmount}</td>
                          <td className="py-3 px-4">
                            <span className={`text-xs px-2 py-1 rounded-full ${
                              booking.status === 'completed' ? 'bg-green-500/20 text-green-400' :
                              booking.status === 'confirmed' ? 'bg-blue-500/20 text-blue-400' :
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {reviews.map((review, index) => (
                    <div key={index} className="bg-black/30 p-4 rounded-lg border border-[#D91CD2]/20">
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2">
                          <span className="font-medium">Course ID: {review.courseId}</span>
                        </div>
                        <div className="flex items-center">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span key={i} className={`text-lg ${i < review.rating ? 'text-yellow-400' : 'text-gray-600'}`}>★</span>
